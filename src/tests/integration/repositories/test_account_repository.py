...
class _TestBaseAccountRepository(TestBaseIntegration):
    @pytest.fixture(autouse=True)
    def setup(self):
        self.account_repository = AccountRepository(session)


class TestFindByIdAccountRepository(_TestBaseAccountRepository):
    def test_find_account_found(self):
        account = AccountFactory()

        found_account = self.account_repository.find_by_id(account.id)
    ...
    def test_find_account_not_found(self):
        ...
        found_account = self.account_repository.find_by_id(account_id)

        assert found_account is None


class TestGetAccountRepository(_TestBaseAccountRepository):
    def test_get_accounts_found(self):
        [AccountFactory() for _ in range(2)]

        accounts = self.account_repository.get()
    ...
    def test_get_accounts_found_filter_by_page_number(self, filters, total_expected_accounts):
        ...
        accounts = self.account_repository.get(**filters)
