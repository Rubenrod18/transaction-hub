...
class _TestBaseTransactionService(TestBaseIntegration):
    @pytest.fixture(autouse=True)
    def setup(self):
        self.transaction_service = TransactionService(session)


class TestFindByIdTransactionService(_TestBaseTransactionService):
    def test_find_transaction_found(self):
        transaction = TransactionFactory()

        found_transaction = self.transaction_service.find_by_id(transaction.id)
    ...
    def test_find_transaction_not_found(self):
        ...
        found_transaction = self.transaction_service.find_by_id(transaction_id)

        assert found_transaction is None


class TestGetTransactionService(_TestBaseTransactionService):
    ...
    def test_get_transactions_found_filter_by_page_number(self, filters, total_expected_transactions):
        ...
        transactions = self.transaction_service.get(**filters)
...
class TestCreateTransactionService(_TestBaseTransactionService):
    def test_create_transaction(self):
        transaction_data = TransactionFactory.build_dict(exclude={'created_at', 'updated_at', 'deleted_at'})

        new_transaction = self.transaction_service.create(**transaction_data)
